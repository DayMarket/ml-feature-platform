...
Запрос выполняется через `feature_stats.task.fetch_rows` — курсор соединения TrinoHook,
а не `hook.get_records`. Последний идёт через `DbApiHook.run`, а тот разбивает SQL на
statement'ы через `sqlparse.split_sql_string`; начиная с sqlparse 0.6.0 группировка
отказывается разбирать больше 10000 токенов (`MAX_GROUPING_TOKENS`). Запрос профиля стоит
примерно 112 токенов на признак, то есть 89 признаков — это 10005 токенов и
`SQLParseError` на клиенте, до Trino запрос не доезжает вовсе. Делить наш SQL не нужно:
`render_stats_query` всегда отдаёт ровно один statement. Понижать `columns_per_query`
ради этого лимита тоже не нужно — это лишние полные сканы партиции, а следующая широкая
таблица упрётся в тот же порог на другом числе колонок.
...
Конкурентные commits других DAG в общую таблицу результатов повторяются после обновления
Iceberg metadata с exponential backoff и jitter. SQL расчёта статистик повторно не запускается.
...
### Несколько партиций одной задачей

`feature_stats.partition_date_template` может отрендериться в список значений через
запятую — так же, как у `dq`. Профили считаются для каждой партиции (каждая —
отдельный скан в Trino) и сохраняются одним commit'ом. `query_timeout_seconds`
ограничивает всю таску, поэтому у витрин с окном пересчёта его нужно поднимать
пропорционально числу дней.
