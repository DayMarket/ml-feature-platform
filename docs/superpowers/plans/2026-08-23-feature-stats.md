...
партиции ради разбиения на партии. Остальные шесть таблиц уже (по числу
числовых признаков) не шире `sku_group_search_conversion_features_v2`, так
что для всех семи таблиц риск "план не строится" снят. **Решение:
`columns_per_query: null` (один запрос на таблицу) остаётся дефолтом для
всех семи таблиц в Task 8; ни для одной не нужно явно задавать
`columns_per_query` в `config.yaml`.**
