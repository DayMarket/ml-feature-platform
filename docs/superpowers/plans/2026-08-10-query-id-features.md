...
к v2 (5 962 465 -> 23 122 598) при среднем размере группы 1.36. Отсечки топ-N нет, пишутся все пары.
...
6. Согласовать фактический объём с владельцами топика `ranking.features.updates` и
   ranking-сервиса: парная таблица на замере от 2026-08-12 даёт 3.88 раза к v2, то есть ~23M строк
   в сутки против ~6M сейчас.
...
6. Бэкфилла справочника не требуется. На 2026-08-12 он содержит 8.39M записей за 15 ежедневных
   прогонов и покрывает 99.98 % различных запросов живого дня и 95.6 % показов; непокрытое — это
   пустой запрос, пунктуация и строки из одних стоп-слов, которые DAG справочника пропускает по
   своей логике и которые уходят в фолбэк.

## Follow-ups из финального ревью

Не блокируют мерж, но зафиксированы, чтобы не потерялись.

1. **Ресурсы парной таблицы.** Профиль `large` (5 экзекьюторов × 8 ядер × 16g,
   `dynamicAllocation` выключен) рассчитан на ~6M строк выхода, а станет ~23M. Проверить на первом
   же прогоне и перепрофилировать при необходимости. Прежняя проекция в ~200M строк была ошибочной
   и снята — см. раздел «Что было измерено неверно» в спеке.
2. **Тесты дельт.** `test_execution_deltas_line_up_with_the_schedule` и его парный аналог проверяют
   наличие `timedelta(hours=1)` и `timedelta(hours=5)` в файле, но не привязывают дельту к
   конкретному `external_dag_id`. Перестановка дельт между сенсорами тест не завалит. Оркестрация —
   единственное место, где тихая регрессия дорога; привязка регуляркой стоит ~5 строк.
3. **DQ на `sku_group_id IS NOT NULL`.** Строки с `NULL sku_group_id` теперь размножаются на всю
   группу вместо одной строки на запрос. Это точное зеркало предсуществующего поведения v2, но
   перед итерацией с аплоадом стоит добавить проверку в сгенерированный DQ.
4. **Групповой тег Airflow.** Тег `query-id` захардкожен в обоих `dag.py`, тогда как AGENTS.md
   требует `dag.group_tag` в `config.yaml`. Апстримный справочник использует `search-query-id`.
   Runtime-эффекта нет, CI не проверяет; при следующем касании этих сущностей стоит свести к одному
   тегу из конфига.
