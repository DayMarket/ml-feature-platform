...
1. **Ресурсы парной таблицы после бэкфилла.** Профиль `large` (5 экзекьюторов × 8 ядер × 16g,
   `dynamicAllocation` выключен) должен справиться с текущими ~20M строк, но не с ~200M, которые
   спека проецирует после бэкфилла справочника на 90 дней. Переснять и перепрофилировать до
   включения любой группы аплоада.
