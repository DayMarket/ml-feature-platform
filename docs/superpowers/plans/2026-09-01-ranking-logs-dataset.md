...
**Решение в этом плане:** `row_count_growth` в конфиг не включается, причина фиксируется комментарием в `config.yaml` и строкой в README. Поддержка недельного baseline в `dq/` — отдельная работа вне этого плана.
