...
выкупаемости (2 889 958). Следствие: у ~7.6 млн sku все семь buyout-колонок
...
- **`sku_n_delivered` и `product_n_delivered` — `integer`**, тогда как в витрине
  ...
  234 462, то есть запас до предела `int4` четырёхзначный. Выгрузка не приводит
  тип явно: значения идут в `COPY` как есть, а целевой `integer` применяет
  PostgreSQL. При переполнении `COPY` падает с ошибкой (`integer out of range`),
  а не молча обрезает значение — это и удовлетворяет требованию «падать, а не
  молча обрезать».
