...
`sample_percent` — параметр `config.yaml`. Плановое значение — **7**; на первый ран закоммичена **1**, чтобы первое исполнение измерило реальную стоимость, а не собрало боевой объём сразу. Хэш от `request_id` даёт
...
| `sample_percent = 7` (плановое) | ~28 млн/день, **~200 млн за ран** |
| `sample_percent = 1` (первый ран) | ~4 млн/день, **~28 млн за ран** |
