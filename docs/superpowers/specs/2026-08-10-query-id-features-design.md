...
**Следствие для дизайна: в целевом режиме отсечка топ-N `sku_group_id` по показам внутри группы
обязательна.** ~200M строк в сутки в Kafka и в ranking-сервис — не тот объём, на который рассчитан
текущий аплоад. `N` подбирается по замеру после бэкфилла; параметр живёт в `config.yaml`, схему
менять не требуется. Концентрация выхода в топ-1000 групп означает, что отсечка срежет основную
часть объёма, почти не задев типичную группу (p99 = 5 запросов).
