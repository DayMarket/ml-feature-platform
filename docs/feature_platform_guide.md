...
Работать с Feature Platform можно с помощью AI-ассистента или полностью вручную. Рекомендуемый вариант для разработки с ассистентом — Codex или Claude: они помогают найти существующие признаки, проверить контракты источников, подготовить код и запустить локальные проверки. AI-ассистент не является обязательным требованием и не заменяет ревью разработчика.

Перед началом работы должны быть настроены:

- доступ к репозиторию `ml-feature-platform`, локальный Git и рабочая ветка, созданная от актуальной `dev`;
- Codex или Claude, если используется разработка с AI-ассистентом; инструкции для Codex находятся в корневом `AGENTS.md`, а для Claude — в `CLAUDE.md`;
- Python 3 для запуска repository scripts и локальных проверок; дополнительные зависимости нужны только для тех проверок или jobs, которые запускаются локально;
- MCP-серверы ClickHouse и Trino с разрешенными пользователю доступами, если для задачи нужно исследовать схемы, партиции, freshness, примеры данных или значения бизнес-полей в соответствующем источнике;
- MCP-сервер GitHub — опционально, для работы с pull request, review comments и статусами CI; без него эти действия можно выполнять через GitHub UI или CLI;
- необходимые инфраструктурные доступы: Airflow connections, source permissions и credentials для локального запуска — только если они требуются конкретной задачей. Секреты нельзя добавлять в репозиторий или передавать в коде и конфигурации сущности.

MCP-доступ не означает автоматического разрешения на просмотр production-данных. Если источник, схема или бизнес-семантика не зафиксированы в задаче или репозитории, ассистент сначала запрашивает разрешение на минимально необходимую MCP-инспекцию. Для ручной разработки действует тот же принцип: источник и его контракт нужно подтвердить до реализации.

Минимальный сценарий начала работы:

1. Обновить локальную `dev` и создать рабочую ветку.
2. Прочитать инструкции своего ассистента (`AGENTS.md` для Codex или `CLAUDE.md` для Claude) и документацию затрагиваемой сущности.
3. Проверить, нет ли уже признака с тем же grain и семантикой.
4. Подтвердить source contract, ownership, alerting, расписание и runtime-параметры, которые требуются для изменения.
5. После реализации выполнить релевантные локальные проверки из этого руководства.

## 2. Что такое сущность платформы
...
## 4. Как понять, какие признаки уже есть
...
## 5. Если нужного источника нет в Feature Platform
...
## 6. Пример: пользователь прислал SQL
...
## 7. Пример: создать на основе существующего silver/gold
...
## 9. Пример вопрос/ответ: какие признаки собираются для отзывов?
...
## 10. Пример вопрос/ответ: как собирается сервис ранжирования?
...
## 11. Правила именования таблиц и признаков
...
## 15. Что происходит после готового кода
...
## 16. Какие PR создаются после merge
...
## 17. Как добавить новую колонку в таблицу
...
## 21. Минимальный шаблон финального ответа по новой таблице
