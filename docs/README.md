...
Разработку можно вести с помощью Codex или Claude либо полностью вручную. Понадобятся доступ к репозиторию, Git, Python 3 и необходимые для конкретной задачи инфраструктурные права. Для исследования источников должны быть настроены MCP-серверы ClickHouse и Trino; MCP-сервер GitHub опционален и упрощает работу с PR, review и CI.

Перед изменениями прочитайте инструкции своего ассистента: корневой `AGENTS.md` для Codex или `CLAUDE.md` для Claude. Настроенный MCP-доступ сам по себе не разрешает просмотр production-данных: неизвестные схемы и бизнес-семантику следует исследовать только после явного согласования. Полный список требований и порядок начала работы приведены в [подробном руководстве](feature_platform_guide.md#1-начало-работы).
