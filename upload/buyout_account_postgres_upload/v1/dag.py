"""Опубликовать последнюю партицию buyout_online_account_features в PostgreSQL сервиса невыкупов.

Нет своего Iceberg-марта и нет своего job/: выгружаются account_id + одна
фича + одна дата витрины buyout_online_account_features плюс четыре
константы. Публикующий код (`publish()`, `column_map`/`constants`) —
общий с buyout_sku_postgres_upload, импортируется отсюда напрямую, без
дублирования job/, как выгрузки, переиспользующие features_service_upload.
"""
...
# неё только конфигом (column_map/constants), а не логикой.
SKU_UPLOAD_DIR = os.path.join(REPO_ROOT, "upload", "buyout_sku_postgres_upload", "v1")
...
def buyout_account_postgres_upload() -> None:
    wait_for_gold_dq = ExternalTaskSensor(
        task_id="wait_for_buyout_online_account_features_dq",
    ...
    def publish_to_postgres(interval_end_value: str) -> None:
        ...
        runtime = _load_module(
            ...
            "buyout_account_features_runtime",
        )
        job = _load_module(
            os.path.join(SKU_UPLOAD_DIR, "job", "upload_postgres.py"),
            "buyout_account_upload_postgres",
        )
        ...
        job.publish(
            ...
            pendulum.now("UTC"),
            min_rows=min_rows,
            column_map=SINK["column_map"],
            constants=SINK["constants"],
        )
...
dag = buyout_account_postgres_upload()
