...
(`row_number() over (partition by category_id, query_text order by date desc, relevance desc) = 1`).
Равная `date` с разным `relevance` бывает, когда в одной категории два `query_id` отличаются
только регистром: на 2026-09-14 так вышло с 5 ключами (`query_id` `faberlic` с
`relevance = 2` и `Faberlic` с `0`), уходит `2`. Это расхождение самой витрины,
справочник его не создаёт.
