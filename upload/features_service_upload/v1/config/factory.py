...
def _build_model_components(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    models = config.get("models") or _default_models(config)
    ...
    for component_models in components_by_root.values():
        ...
        for group_name in sorted(requested_features_by_group):
            ...
            ordered_features.extend(
                sorted(requested_features - set(catalog_features))
            )
...
def get_deployment(
    ...
    replacements = {
        ...
        "<run_date>": "{{ ds }}",
        "<config_path>": "/git/repo/upload/features_service_upload/v1/config.yaml",
