...
Если новая модель использует только свои feature groups, DAG создаст для нее независимый upload task со своими DQ-сенсорами. Если новая модель использует хотя бы одну общую feature group с другой моделью, эти модели попадут в один upload component, который дождется общий набор DQ и загрузит union признаков по shared feature groups. Если новая модель использует только уже существующие feature groups и признаки, новые feature groups создавать не нужно.
