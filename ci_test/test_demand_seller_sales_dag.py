...
ENTITY = ROOT / "layers/silver/sku_id_seller_key/demand_seller_sales_observed_daily/v1"
...
def test_prepare_and_write_have_whole_task_guard():
    tree = ast.parse((ENTITY / "dag.py").read_text())
    owner = next(n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name == "seller_sales_dag")
