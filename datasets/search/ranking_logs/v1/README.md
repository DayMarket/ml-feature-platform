...
`dataset.sample_percent` задаётся в процентах и может быть дробным: **7** — это 7%,
**0.01** — одна сотая процента. Шаг сетки задаёт `HASH_BUCKETS = 10000`, поэтому
минимальная представимая доля — 0.01; на всём, что мельче, `query.sample_threshold`
падает явным `ValueError`, а не пишет пустую партицию.

Сейчас в `config.yaml` стоит **0.01**: ~40 тыс. строк в сутки и ~280 тыс. за ран.
Это разведочная доля — её задача измерить реальные метрики Spark, а не собрать
боевой объём. До неё стояла 1 (~4 млн строк в сутки, ~28 млн за ран), но ран на
ней был снят по таймауту. Плановая доля 7% возвращается после измерения, вместе с
пересмотром профиля ресурсов `search_dataset`.

Важно при чтении замеров: снижение доли не удешевляет самый дорогой скан.
`model_input` в источнике — `map`, ключ из него не проецируется, поэтому
145-мерный вектор читается при каждом ране, сколько бы строк ни писалось.
`sample_percent` режет то, что после скана: shuffle, джойны обогащения и запись.
