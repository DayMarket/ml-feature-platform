...
Первый ран — исследовательский, `dataset.sample_percent` в `config.yaml` снижен с плановых 7 до 1
для измерения реальных метрик Spark перед расширением объёма. При текущих 1%: порядка 4 млн строк
в сутки и ~28 млн за недельный ран. При плановых 7%: ~28 млн строк в сутки и ~200 млн за ран.
Доля повышается только после первого рана, вместе с переоценкой профиля ресурсов `search_dataset`.
