...
def render_source_yaml(
    ...
    # Только lineage: DQ-тесты и freshness живут в таске `dq` DAG'а-владельца
    # (пакет `dq/`), а не в dbt-trino. Здесь рендерится source-блок и primary key
    # как документация грейна — ни `tests:`, ни `freshness:`.
    lines.extend(
        [
