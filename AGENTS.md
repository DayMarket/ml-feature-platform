...
- DAGs that depend on feature-platform tables must wait for the `dq` task of the DAG that owns the table (`external_task_id="dq"`), not for the write task and not for the retired `dbt.source.trino.ml_feature_platform_*.dq` DAG. Those DAGs are no longer generated for new tables, so a sensor naming one fails on `check_existence` or hangs to timeout; `ci_test/test_dq_task_wiring.py` blocks the reference.
...
- Ranking-service publication: `upload/**/config.yaml` and `upload/**/ranking_service_input.yaml`. There are four Kafka upload DAGs today: `features_service_upload` (search ranking, `04:00 UTC`), `dynamic_pricing_inference_upload` (every 3 hours), `buyout_features_upload` (buyout model, `07:00 UTC`) and `query_category_relevance_upload` (`query_category_relevance_expanded` partition, `04:00 UTC`). Job code and the SparkApplication template live only in `upload/features_service_upload/v1`; the other three carry just `config.yaml`, `dag.py` and `config/factory.py`. A new upload DAG is the right shape when the producer's schedule cannot give a non-negative `dependency_execution_delta_minutes` against an existing upload DAG, when owner/alert routing differs, or when the group's `source.read_mode` differs from the existing DAG's groups.
...
rg -n "<column_name>" layers/**/migrations upload
...
- Inspect every `upload/**/config.yaml` and `upload/**/ranking_service_input.yaml` for downstream usage and required feature order.
...
- Dataset tables must not be uploaded to ranking-service, inference services, or online serving systems from this repository. Do not edit any `upload/**/config.yaml` or `upload/**/ranking_service_input.yaml` for a dataset output.
...
- If the feature is published, remove it from the owning `upload/**/config.yaml` and update the matching `ranking_service_input.yaml` only after confirming serving compatibility and feature order changes.
...
- Recsys Gold contracts declare `table.feature_namespace`, and every physical feature column uses `<feature_namespace>__<base_name>`, for example `ACCOUNT_PRODUCT__n_clicks_7d` or `ACCOUNT_L1__n_clicks_7d`. Keep the base name in lower snake case without repeating the entity abbreviation (`pid_`, `l1_`...`l5_`, `bid_`, `sid_`). Entity key columns such as `product_id`, `brand_id`, `shop_id`, and `l1_category_id` keep their canonical names and do not receive the namespace.
- Recsys recency features measured in days use exact fractional day differences stored as `DOUBLE`. Do not apply `CEIL`, `FLOOR`, `ROUND`, or integer casts unless an individual feature contract explicitly requires discrete calendar-day semantics.
...
- Downstream-DAG'и ждут таску `dq` DAG'а-владельца таблицы: `external_dag_id=<dag id владельца>`, `external_task_id="dq"`. Ссылки на `dbt.source.trino.ml_feature_platform_*.dq` из репозитория убраны полностью, и `ci_test/test_dq_task_wiring.py` не даёт им вернуться. Раздел `## 4` файла `docs/feature_platform_map.md` показывает, что легаси-рёбер не осталось; если они появятся снова, генератор карты нарисует их пунктиром.
- `execution_delta` считается от расписаний владельца и потребителя, а не от прежних 01:00 легаси dbt-DQ: логическая дата потребителя минус дельта обязана попасть ровно в ран владельца, записавший нужную партицию. Переводя сенсор на новый контракт, дельту надо пересчитать, а не перенести — у владельцев, которые идут не в 01:00 (`sku_stock_daily` и пять silver-энтити `h3_index` идут в 00:00), она меняется.
...
`scripts/sync_dbt_sources.py` больше не генерирует DQ-тесты в `dbt-trino`: он пишет в `sources.yaml` только source-блок с `meta.owner` и колонками `table.primary_key` — ради lineage и каталога. Ни `tests:`, ни `freshness:`, ни `loaded_at_field:` в рендер не попадают; `ci_test/test_sync_dbt_sources.py` это фиксирует. Единственный источник DQ — таска `dq` в DAG'е владельца таблицы (пакет `dq/`). Новую проверку заводи именем в `dq.tests` конфига энтити, а не правкой генератора и не PR'ом в `dbt-trino`.

Записи `tests:`/`freshness:`, уже лежащие в `sources.yaml` репозитория `dbt-trino` от прошлых прогонов, генератор не трогает: их снимает отдельный согласующий PR в чужом репозитории. Пока он не влит, у таких таблиц DQ считается дважды — в Airflow и в dbt-DQ-DAG'е.
...
- `scripts/sync_dbt_sources.py` must discover repository-managed layer and dataset configs and write source blocks to `models/ml_feature_platform/sources.yaml`. Source blocks only: rendering `tests:` or `freshness:` into dbt is a regression, DQ belongs to the `dq` task. If it only discovers layer configs, update it before adding the first dataset table.
...
- Supported entity keys are `sku_group_id`, `query`, `account_id`, `sku_id`, `query,sku_group_id`, `category_id,sku_group_id`, `promotion_id,sku_group_id`, `account_id,category_id`, and `category_id,query_text`. The last one maps to collection `SKU_GROUP_CATEGORY_TO_QUERY` with proto fields renamed per collection (`category_id` → `skuGroupCategoryId`, `query_text` → `query`) in `ENTITY_KEY_ARGUMENTS`.
- `source.read_mode` selects rows: absent — partition `date = run_date`; `latest_timestamp` — the latest snapshot of a timestamp-keyed source; `full_table` — the whole table, one row per entity key from its latest `date`, ties on `date` going to the largest feature values in feature order (the source primary key must contain `date`). `full_table` resends every key on each run, so size the upload resources by table size, not partition size.
- `source.query_id_dictionary` (`{schema, table}`, only with `read_mode: full_table` and a `query_text` entity key) adds, before the latest-date selection, a copy of each source row for every `query_text` of the same `query_id` in the dictionary table, then applies `lower` to the final `query_text` and nothing else. Both the source and the dictionary must declare `query_id` (and the dictionary `query_text`) in their migrations; the validator enforces this. Today only `query_category_relevance_upload` uses it, with `gold.feature_platform_search_query_id`.
...
- `source.dependency_task_id` must be `"dq"` for every repository-managed source: the upload waits for the owning DAG's `dq` task, never for the whole DAG and never for the legacy `dbt.source.trino.*` DQ DAG. A successful write with a failed DQ must not publish features. `scripts/validate_ranking_upload_configs.py` enforces this; only `source.external: true` groups name a different producer task, because they follow the owning team's contract. The single other exception is an owner's explicit decision to run the source DAG without a `dq` task: the group then names the producer's write task and states `source.dq_waiver_reason`; the validator rejects the waiver as soon as the source `dag.py` calls `build_dq_task`.
