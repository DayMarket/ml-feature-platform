...
- `table.meta.create_dbt_pr` and `table.meta.create_maintenance_pr` are optional bool flags controlling whether CI creates missing downstream PRs in dbt sources and Iceberg maintenance. Both default to `true` when absent. Set a flag to `false` only when the table must not be onboarded automatically; CI must not use this as a destructive delete signal for existing downstream records.
