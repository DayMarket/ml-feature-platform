...
- `layers/` is an entity tree grouped by primary key, not a shared-library root. Keep an entity's runtime, query, write, and orchestration code inside `layers/<layer>/<primary_key_group>/<entity>/vN/`. Derive `primary_key_group` from `table.primary_key` after removing every column whose output type in the migration is `DATE` or any `TIMESTAMP` type, including names such as `date`, `dt`, `snapshot_date`, and `calculated_at`; temporal columns must never appear in a layer directory name even when they are part of the primary key. Preserve the order of the remaining columns and join their names with underscores. If no non-temporal key columns remain, ask the user to confirm the directory group instead of inventing one. The `sku_group_install` entity is the sole semantic exception and uses `sku_group_id_query_category`. Do not create `layers/_common`, `layers/_commons`, or another cross-entity utility package there. If reuse is genuinely needed, first identify an existing approved top-level shared location or ask the user to approve a repository-wide abstraction; do not introduce one implicitly.
