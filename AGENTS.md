...
- Recsys Gold contracts may declare `table.feature_namespace` for the logical model/serving namespace. Keep physical Iceberg feature columns in lower snake case without repeating the entity abbreviation (`pid_`, `l1_`...`l5_`, `bid_`, `sid_`). The downstream feature name is `<feature_namespace>__<physical_column>`, for example `ACCOUNT_PRODUCT__n_clicks_7d` or `ACCOUNT_L1__n_clicks_7d`. Entity key columns such as `product_id`, `brand_id`, `shop_id`, and `l1_category_id` keep their canonical names and do not receive the namespace.
