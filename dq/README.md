...
Витрина с атомарной посуточной записью может передать фабрике
`range_receipt_task_id`. Тогда одна терминальная таска `dq` берёт список дат и дневных
receipts из writer XCom и проверяет каждую дату. До и после проверки она убеждается,
что current Iceberg snapshot не сменился, и сохраняет обычные результаты DQ отдельно
по дням. Остальные DAG используют фабрику
без этого аргумента и сохраняют прежнее поведение. Range-режим требует дневной
partition, `warmup_days: 0`, базовые проверки ключа/уникальности/объёма и error severity.
Freshness и growth остаются включёнными в конфиге дневной витрины. Executor выполняет
их для дня `UTC date(day_receipt.ingested_at) - 1`; для более старых дат диапазона
исключает только эти два теста. Привязка к capture сохраняет live-проверки при retry
DQ на следующие сутки. Ключи, объём, бизнес-инварианты и сверка writer receipt
проверяются для всех дат. Отсутствие baseline у live-growth блокирует приёмку;
перед последним днём должны быть загружены и предыдущие дни. Дефолтный порог growth —
20% в обе стороны; он обнаруживает резкое изменение количества строк, но сам по себе
не доказывает полноту источника.
