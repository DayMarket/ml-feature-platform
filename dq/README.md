...
Если DAG одним запуском перезаписывает несколько дней (окно пересчёта или ручной
диапазон), `dq.partition_date_template` может отрендериться в список значений через
запятую, например `{{ ti.xcom_pull(task_ids="write") | join(",") }}`, где `write`
возвращает список ISO-дат. Таска `dq` прогоняет все тесты для каждой партиции по
возрастанию, сохраняет результаты одним commit'ом (перезаписываются строки этого
DAG'а за проверенные даты) и падает, если хотя бы одна партиция не прошла.
Фабрика и её вызов в DAG'е при этом не меняются.
