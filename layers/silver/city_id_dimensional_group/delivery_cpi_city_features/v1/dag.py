"""Materialize the city delivery CPI snapshot from Trino to Iceberg."""
...
def delivery_cpi_city_features_dag() -> None:
    @task(executor_config=_executor_config())
    def materialize(interval_end_value: str) -> None:
        runtime = _load_module("runtime.py", "delivery_cpi_city_features_runtime")
        query = _load_module("query.py", "delivery_cpi_city_features_query")
        ...
        # Партиция date равна дате конца интервала: окно закрыто строго до неё,
        # поэтому запуск в 03:00 UTC уже видит полные сутки date - 1.
...
dag = delivery_cpi_city_features_dag()
