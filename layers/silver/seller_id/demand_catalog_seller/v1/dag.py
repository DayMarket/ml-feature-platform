"""Материализовать полный seller-каталог; ручной capture использует того же owner."""
...
from layers.silver.seller_id.demand_catalog_seller.v1.job.budget import (  # noqa: E402
    configured_limits,
    run_guard,
)
...
def catalog_dag():
    ...
    def prepare_request():
        from layers.silver.seller_id.demand_catalog_seller.v1.job.requests import capture_request
    ...
    def write_catalog(request):
        from layers.silver.seller_id.demand_catalog_seller.v1.job.orchestration import execute_capture
        context = get_current_context()
        with owner_guard(context):
