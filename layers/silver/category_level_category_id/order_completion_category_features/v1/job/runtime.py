"""Entity-local Airflow/Python runtime for the category-level order-completion snapshot."""
...
def lookback_days(config: Mapping[str, Any]) -> int:
    source = config.get("source")
    if not isinstance(source, Mapping):
        raise ValueError("config.yaml must contain a source mapping")

    raw = source.get("lookback_days")
    try:
        days = int(str(raw).strip())
    except (TypeError, ValueError) as exc:
        raise ValueError(
            f"config source.lookback_days must be an integer, got {raw!r}"
        ) from exc
    if days <= 0:
        raise ValueError(f"config source.lookback_days must be positive, got {days}")
    return days
...
def require_non_empty(frame, partition_date: date) -> None:
    """Guard against replacing a good partition with an unpublished source snapshot."""
    if frame.empty:
        raise RuntimeError(
            "Source query returned no rows for analyze_date="
            f"{partition_date}; refusing to overwrite the partition."
        )
