...
Silver-пайплайн строит дневную предагрегацию событий на уровне пары `account_id` и L1 `category_id`.

## Выходная таблица

- `iceberg.silver.feature_platform_account_l1_event_w_imps_counts`
...
Для каждого события выбирается категория: `event.category_id`, если она больше нуля, иначе `category_id` товара из `iceberg.silver.product`. Затем категория приводится к L1 через справочник категорий.
...
DAG `feature-platform.layers.silver.account_id_category_id.account_l1_event_w_imps_counts` запускается ежедневно в 00:00 по `Asia/Tashkent` (`schedule_interval="0 19 * * *"` в UTC). Таблица получает стандартные сгенерированные dbt DQ-тесты по primary key после синка источников.
