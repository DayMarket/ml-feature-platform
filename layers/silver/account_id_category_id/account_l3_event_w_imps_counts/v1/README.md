...
Silver-пайплайн строит дневную предагрегацию событий на уровне пары `account_id` и L3 `category_id`.

## Выходная таблица

- `iceberg.silver.feature_platform_account_l3_event_w_imps_counts`
...
Для каждого события выбирается категория: `event.category_id`, если она больше нуля, иначе `category_id` товара из `iceberg.silver.product`. Затем категория приводится к L3 через справочник категорий. Если L3 отсутствует или меньше/равна нулю, используется L2, а при отсутствии L2 - L1.
...
DAG `feature-platform.layers.silver.account_id_category_id.account_l3_event_w_imps_counts` запускается ежедневно в 00:00 по `Asia/Tashkent` (`schedule_interval="0 19 * * *"` в UTC). Таблица получает стандартные сгенерированные dbt DQ-тесты по primary key после синка источников.
