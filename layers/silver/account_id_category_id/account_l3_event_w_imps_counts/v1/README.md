...
DAG `feature_platform_account_l3_event_w_imps_counts_silver_dag` запускается ежедневно в 00:00 по `Asia/Tashkent` (`schedule_interval="0 19 * * *"` в UTC). Таблица получает стандартные сгенерированные dbt DQ-тесты по primary key после синка источников.
