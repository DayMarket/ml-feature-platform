...
@dag(
    ...
    dag_id="feature-platform.layers.silver.account_id_category_id.account_l2_event_w_imps_counts",
)
def collect_silver_account_l2_event_w_imps_counts():
    SparkKubernetesOperator(
        execution_timeout=timedelta(hours=10),
        task_id="getting_account_l2_event_w_imps_counts",
        namespace="svc-data-spark-jobs",
        application_file=get_deployment(
            ".",
            "fetch_silver_account_l2_event_w_imps_counts.yaml",
        ),
        kubernetes_conn_id="spark_k8s",
    )


dag = collect_silver_account_l2_event_w_imps_counts()
