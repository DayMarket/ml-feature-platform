...
PROOF_KEY = 'demand-restored-checkpoint-v1'
...
def verify_proof(table, proof, snapshot_id):
    ...
    for first, last, size, digest in proof['batches']:
        lower = Or(GreaterThan('sku_id', first[0]),
                   And(EqualTo('sku_id', first[0]), GreaterThanOrEqual('estimate_kind', first[1])))
        upper = Or(LessThan('sku_id', last[0]),
                   And(EqualTo('sku_id', last[0]), LessThanOrEqual('estimate_kind', last[1])))
        part = And(scope, lower, upper)
        actual = table.scan(snapshot_id=snapshot_id, row_filter=part, limit=size + 1).to_arrow()
        actual = actual.sort_by([('sku_id', 'ascending'), ('estimate_kind', 'ascending')])
        if actual.num_rows != size or fingerprint(actual) != digest:
            raise ReadBackMismatch('Read-back E3 не совпал с исходной порцией')
        content.update(actual)
    try:
        content.verify(proof['source_day'])
    except ValueError as error:
        raise ReadBackMismatch('Read-back E3 не совпал с source manifest') from error
...
def resume_day(config, catalog, *, day, selected, run, manifest, version):
        ...
        if PROOF_KEY in properties:
            ...
            if proof['date'] == day.isoformat():
                if (proof['source_manifest_id'] != manifest or proof['source_contract_version'] != version
                        or proof.get('source_day') != expected):
                    return None
                try:
