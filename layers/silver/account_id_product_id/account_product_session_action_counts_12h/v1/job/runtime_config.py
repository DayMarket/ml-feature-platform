...
def load_source_settings(config_path: Path | None = None) -> SourceSettings:
    ...
    events_table = str(source.get("events_table", "")).strip()
    if not events_table:
        raise ValueError(
            f"{config_path}: source.events_table must be a non-empty string"
        )

    event_types = tuple(
        event_type.strip()
        for event_type in str(source.get("event_types", "")).split(",")
        if event_type.strip()
    )
    if not event_types:
        raise ValueError(
            f"{config_path}: source.event_types must not be empty"
        )
    if len(event_types) != len(set(event_types)):
        raise ValueError(
            f"{config_path}: source.event_types must contain unique values"
        )
