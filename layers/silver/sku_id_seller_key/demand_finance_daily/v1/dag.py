"""Записать дневные финансовые события; ручная история использует того же owner."""
...
from layers.silver.sku_id_seller_key.demand_finance_daily.v1.job.budget import (  # noqa: E402
    configured_limits,
    run_guard,
)
...
def finance_dag():
    ...
    def prepare_request():
        from layers.silver.sku_id_seller_key.demand_finance_daily.v1.job.requests import (
    ...
    def write_range(request):
        from layers.silver.sku_id_seller_key.demand_finance_daily.v1.job.orchestration import (
            execute_range,
        )
