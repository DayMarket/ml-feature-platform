"""Атомарно заменить один финансовый день ограниченными Arrow-порциями и сверить запись."""
...
def write_day(config, catalog, batches, *, day, expected_rows, manifest, version,
              ingested_at, verify_source, source_signature=None):
    ...
    with table.transaction() as transaction:
        ...
        if written != expected_rows:
            raise ValueError('Неполный поток finance')
    ...
    if snapshot is None:
        raise RuntimeError('Нет snapshot после finance commit')
