"""Записать дневные продажи продавцов; ручная история использует того же owner."""
...
from layers.silver.sku_id_seller_key.demand_seller_sales_observed_daily.v1.job.budget import (  # noqa: E402
    configured_limits,
    run_guard,
)
...
def seller_sales_dag():
    @task(task_id="prepare_request", multiple_outputs=False)
    def prepare_request():
        from layers.silver.sku_id_seller_key.demand_seller_sales_observed_daily.v1.job.requests import (
    ...
    def write_range(request):
        from layers.silver.sku_id_seller_key.demand_seller_sales_observed_daily.v1.job.orchestration import (
            execute_range,
        )
