"""Подготовить seller-день, сохраняя raw/USD, seller attribution и exact SKU totals."""
...
RAW_COLUMNS = ("date","sku_id","seller_key","seller_id","sku_sales_orders","sku_sales_order_items","sales_units","sales_order_items","sales_orders","sales_gmv","sales_payment_value","sales_full_value","sales_seller_promo_value","sales_marketplace_promo_value","sales_gmv_fbo","sales_gmv_fbs","sales_gmv_dbs","sales_gmv_other","sales_gmv_unknown","sales_units_fbo","sales_units_fbs","sales_units_dbs","sales_units_other","sales_units_unknown","sales_gmv_usd","sales_payment_value_usd","sales_full_value_usd","sales_seller_promo_value_usd","sales_marketplace_promo_value_usd","sales_gmv_fbo_usd","sales_gmv_fbs_usd","sales_gmv_dbs_usd","sales_gmv_other_usd","sales_gmv_unknown_usd","source_updated_at",)
REQUIRED = ("date","sku_id","seller_key","sku_sales_orders","sku_sales_order_items","source_updated_at","fx_rate_source","fx_captured_at","source_manifest_id","source_contract_version","ingested_at",)
...
def validate_schema(schema):
    expected = {
        ...
        "seller_id": pa.int64(),
        "sku_sales_orders": pa.int64(),
        "sku_sales_order_items": pa.int64(),
    ...
    for name, kind in expected.items():
        ...
        if field.nullable != (name not in REQUIRED):
            raise ValueError(f"Nullable не совпадает с миграцией: {name}")
...
def prepare_batch(raw, schema, *, day, fx, manifest, version, ingested_at):
    ...
    for name in RAW_COLUMNS:
        ...
        if name == "seller_key" and not (pa.types.is_string(source.type) or pa.types.is_large_string(source.type)):
            raise ValueError("seller_key должен быть исходной строкой")
...
def validate_batch(batch, schema, *, day):
    ...
    for row in batch.to_pylist():
        ...
        if (seller is None and row["seller_key"] != "unknown") or (
                seller is not None and (seller <= 0 or row["seller_key"] != f"seller:{seller}")):
            raise ValueError("Неверная seller attribution: 0/NULL источника должны быть unknown/NULL")
        for name in ("sales_orders", "sales_order_items"):
            own, control = row[name], row[f"sku_{name}"]
            if own is None or own <= 0 or control <= 0 or own > control:
                raise ValueError("Неверный seller distinct или контрольный SKU итог")
        if (row["sales_orders"] > row["sales_order_items"]
                or row["sku_sales_orders"] > row["sku_sales_order_items"]):
