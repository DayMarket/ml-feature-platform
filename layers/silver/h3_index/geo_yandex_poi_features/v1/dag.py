"""Materialize the geo_yandex_poi_features silver Iceberg table from ClickHouse."""
...
@dag(
    ...
    max_active_runs=1,
    tags=["feature-platform", CONFIG["dag"]["group_tag"], "operations", "silver", "h3", "geo", "poi"],
...
def geo_yandex_poi_features_dag() -> None:
    @task()
    def materialize(partition_value: str) -> None:
        runtime = _load_module("runtime.py", "geo_yandex_poi_features_runtime")
        query = _load_module("query.py", "geo_yandex_poi_features_query")
...
dag = geo_yandex_poi_features_dag()
