...
@dag(
    ...
    max_active_runs=1,
    tags=["spark", "feature-platform", dag_settings["team_tag"], "silver", "query-skg", "linear-score"],
    is_paused_upon_creation=True,
    schedule_interval="0 1 * * *",
    start_date=datetime(2026, 1, 1, 0, 0, 0),
    dag_id="feature_platform_query_skg_daily_linear_score_silver_dag",
)
def collect_silver_query_skg_daily_linear_score():
    SparkKubernetesOperator(
        execution_timeout=timedelta(hours=10),
        task_id="getting_query_skg_daily_linear_score",
        namespace="svc-data-spark-jobs",
        application_file=get_deployment(
            ".",
            "fetch_silver_query_skg_daily_linear_score.yaml",
        ),
        kubernetes_conn_id="spark_k8s",
    )


dag = collect_silver_query_skg_daily_linear_score()
