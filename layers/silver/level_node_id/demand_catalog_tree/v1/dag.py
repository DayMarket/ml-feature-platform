"""Материализовать полный tree-каталог; ручной capture использует того же owner."""
...
from layers.silver.level_node_id.demand_catalog_tree.v1.job.budget import (  # noqa: E402
    configured_limits,
    run_guard,
)
...
SOURCE_CONFIG_PATH = str(Path(REPO_ROOT) / CONFIG["inputs"]["sku_config"])
...
def catalog_dag():
    ...
    def prepare_request():
        from layers.silver.level_node_id.demand_catalog_tree.v1.job.requests import capture_request
    ...
    def write_catalog(request):
        from layers.silver.level_node_id.demand_catalog_tree.v1.job.orchestration import execute_capture
        context = get_current_context()
        with owner_guard(context):
