"""Дерево категорий текущего SKU-каталога: полная замена, строится в Trino."""
...
SKU_CONFIG_PATH = str(Path(REPO_ROOT) / CONFIG["inputs"]["sku_config"])
SKU_CONFIG = load_config(SKU_CONFIG_PATH)
# SKU-каталог запускается в то же время (04:00 UTC): logical date совпадает.
SKU_DQ_DELTA = timedelta(minutes=0)
...
def catalog_tree_dag():
    @task.branch(task_id="upstream_gate")
    def upstream_gate() -> list[str]:
        """Scheduled ждёт DQ SKU-каталога; ручной запуск читает текущий SKU-каталог."""
        run_type = get_current_context()["dag_run"].run_type
        if str(getattr(run_type, "value", run_type)) == "scheduled":
            return ["wait_for_sku_dq", "write"]
        return ["write"]

    @task(task_id="write", trigger_rule="none_failed")
    def write() -> dict:
        from layers.silver.level_node_id.demand_catalog_tree.v1.job.runtime import load

        return load(CONFIG, SKU_CONFIG, REPO_ROOT, run_id=get_current_context()["run_id"])

    gate = upstream_gate()
    sku_ready = ExternalTaskSensor(
        task_id="wait_for_sku_dq",
        external_dag_id=SKU_CONFIG["dag"]["id"],
        external_task_id="dq",
        execution_delta=SKU_DQ_DELTA,
