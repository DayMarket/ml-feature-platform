"""Entity-local Airflow/Python runtime for the daily SKU dynamic-pricing aggregate."""
...
def previous_utc_date(value: str) -> date:
    """Партиция — предыдущие закрытые UTC-сутки: последний батч дня приходит ~23:08 UTC."""
...
def normalize_list_column(frame, column: str):
    """clickhouse-driver отдает Array(Int64) кортежами, Arrow принимает только списки."""
    if column in frame.columns:
        frame[column] = frame[column].map(
            lambda value: list(value) if value is not None else None
        )
    return frame
