...
def _get_shared_deployment_path(deployment_name: Optional[str]) -> str:
    ...
    if not template_path:
        raise FileNotFoundError(
            "Local deployment file is missing and spark.template_path is not configured"
        )
...
def _get_resources_config() -> Dict[str, Any]:
    ...
    resources_path = (
        resources.get("path", "config/resources.yaml")
...
def _fill_arguments(
    deployment_content: str, deployment_name: Optional[str] = None
) -> str:
    dag_config = _get_dag_config()
    spark_config = _get_spark_config(deployment_name)
    ...
    from_to_replacement = {
        ...
        "<spark_event_log_bucket_name>": str(
            resources_config["spark_event_log_bucket"]
        ),
        ...
        "<s3_search_research_secret_key>": s3_search_research_connection[
            "aws_secret_access_key"
        ],
        "<s3_search_research_access_key>": s3_search_research_connection[
            "aws_access_key_id"
        ],
    }
