"""Текущий SKU-каталог: полная замена из dict.sku, категорий, MDM и seller-каталога."""
...
# Seller-каталог запускается в то же время (04:00 UTC): logical date совпадает.
...
def catalog_sku_dag():
    ...
    def upstream_gate() -> list[str]:
        """Scheduled ждёт DQ seller-каталога; ручной запуск читает текущий seller-каталог."""
    ...
    def write() -> dict:
        from layers.silver.sku_id.demand_catalog_sku.v1.job.runtime import load

        return load(CONFIG, SELLER_CONFIG, run_id=get_current_context()["run_id"])
