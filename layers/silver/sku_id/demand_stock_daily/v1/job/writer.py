"""Атомарно заменить один EOD-день ограниченными Arrow-порциями и сверить запись."""
...
def write_day(config, catalog, batches, *, day, expected_rows, manifest, version,
    ...
    with table.transaction() as transaction:
        ...
        if written != expected_rows:
            raise ValueError('Неполный поток stock')
        if verify_source() is not True:
            raise ValueError('Source изменился или не подтверждён перед commit')
    ...
    if snapshot is None:
        raise RuntimeError('Нет snapshot после stock commit')
