...
PROOF_KEY = 'demand-sales-checkpoint-v1'
...
def verify_proof(table, proof, snapshot_id):
    ...
    for first, last, size, digest in proof['batches']:
        ...
        if actual.num_rows != size or fingerprint(actual) != digest:
            raise ReadBackMismatch('Read-back sales не совпал с исходной порцией')
...
def resume_day(config, catalog, *, day, manifest, version, source_binding=None):
        ...
        if PROOF_KEY in properties:
            ...
            if proof['date'] == day.isoformat():
                if proof['source_manifest_id'] != manifest or proof['source_contract_version'] != version:
                    return None
                if source_binding is not None and proof.get('source_binding') != source_binding:
                    return None
                try:
