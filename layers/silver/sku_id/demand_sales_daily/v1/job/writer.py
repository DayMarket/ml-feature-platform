"""Атомарно заменить один день продаж ограниченными Arrow-порциями и сверить запись."""
...
def write_day(config, catalog, batches, *, day, expected_rows, manifest, version,
              ingested_at, verify_source, source_signature=None, source_binding=None,
    ...
    if expected_metadata_location is not None and table.metadata_location != expected_metadata_location:
        raise ValueError('SKU-sales изменился после preflight источника')
    ...
    if source_binding is not None:
        if not isinstance(source_binding, dict) or not source_binding:
            raise ValueError('Нужен непустой exact source binding')
        proof['source_binding'] = json.loads(json.dumps(source_binding, allow_nan=False))
    with table.transaction() as transaction:
        ...
        if written != expected_rows:
            raise ValueError('Неполный поток sales')
    ...
    if snapshot is None:
        raise RuntimeError('Нет snapshot после sales commit')
