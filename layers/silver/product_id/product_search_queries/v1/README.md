...
- `search_queries` - массив структур `{search_query, uniq_installs}`, отсортированный по `uniq_installs DESC`, затем по `search_query ASC` для стабильного порядка.
...
Те же источники, модель и длина окна зафиксированы в `config.yaml` в блоке `source`: `engine: spark_iceberg`, `model_name: search_unified_model_v6`, `lookback_days: 14`.
...
Пайплайн фильтрует события по `model_name = 'search_unified_model_v6'`, непустому `ranking_candidates`, разворачивает массив кандидатов в `sku_group_id`, соединяет его с `iceberg.silver.sku` и оставляет уникальные пары `search_query, product_id`. `uniq_installs` считается как `COUNT(DISTINCT install_id)` по `search_query` за то же 14-дневное окно, как в исходном SQL.
...
DAG `feature-platform.layers.silver.product_id.product_search_queries` запускается ежедневно в `03:00 UTC`, стартовая дата - `2026-06-29 00:00 UTC`, чтобы поддержать snapshot за текущую дату `2026-06-29`.
...
В `config.yaml` включены `table.meta.create_dbt_pr: true` и `table.meta.create_maintenance_pr: true`, поэтому CI должен создать PR в dbt sources и PR в Iceberg maintenance, если соответствующих записей еще нет.
