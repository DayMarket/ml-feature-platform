...
Его `run_id` строится из `run_after`, то есть конца календарного data interval, а
sensor находит тот же запуск по logical date — началу интервала. Если старый успешный
DQ ещё не публиковал payload, receipt читается из `write_calendar` строго того же
`dag_id/run_id`; успешное состояние DQ предварительно подтверждает sensor. Для ручного
