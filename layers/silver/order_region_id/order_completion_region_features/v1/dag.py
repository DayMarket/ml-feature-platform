"""Materialize region-level order completion shares from Trino to Iceberg."""
...
def order_completion_region_features_dag() -> None:
    @task(executor_config=_executor_config())
    def materialize(interval_end_value: str) -> None:
        runtime = _load_module("runtime.py", "order_completion_region_features_runtime")
        query = _load_module("query.py", "order_completion_region_features_query")
        ...
        # Партиция date равна analyze_date снапшота history_order_items,
        # который публикуется в 19:00 UTC того же дня.
...
dag = order_completion_region_features_dag()
