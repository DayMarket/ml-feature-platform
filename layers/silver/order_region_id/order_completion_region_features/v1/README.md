# iceberg.silver.feature_platform_order_completion_region_features
...
- Таблица: `iceberg.silver.feature_platform_order_completion_region_features`.
- DAG: `feature-platform.layers.silver.order_region_id.order_completion_region_features`
  (`layers/silver/order_region_id/order_completion_region_features/v1/dag.py`).
...
- `order_region_id IS NULL` отбрасывается (на замере — 4 строки).
- Кардинальность низкая: на `2026-08-01` — 14 регионов. Freshness- и row-count-проверки надо
  трактовать с учётом этого объёма.
