...
Воркеры только читают Elasticsearch и возвращают строки в parent task. Parent task очищает партицию `date`
одним PyIceberg `overwrite` пустым набором перед первым append-ом с данными, затем последовательно append-ит
чанки в эту партицию. Если Elasticsearch не вернул строк, партиция очищается в конце run-а, чтобы старые данные
за дату не остались в таблице. При retry DAG снова очищает партицию перед первой записью, поэтому partial append
предыдущей попытки не накапливается. PyIceberg `overwrite` и `append` дополнительно обернуты в retry на
table-level lock Hive catalog: это защищает от временного lock со стороны DQ, maintenance или другого writer
таблицы.
