...
`job/runtime.py`. Образ задачи: `ghcr.io/daymarket/airflow:3.1.8-python3.11-ml-2`, ресурсы pod: 24 CPU и
16Gi memory.
...
накапливается. PyIceberg `overwrite` и `append` дополнительно обернуты в retry на table-level lock Hive catalog:
это защищает от временного lock со стороны DQ, maintenance или другого writer таблицы.
