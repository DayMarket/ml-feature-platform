"""Materialize the account lifetime facts snapshot from ClickHouse to Iceberg."""
...
def account_lifetime_facts_dag() -> None:
    @task(executor_config=_executor_config())
    def materialize(interval_end_value: str) -> None:
        runtime = _load_module("runtime.py", "account_lifetime_facts_runtime")
        query = _load_module("query.py", "account_lifetime_facts_query")
        ...
        # Партиция date равна дате конца интервала: снимок состояния аккаунтов
        # на эту дату, окно отбора популяции отсчитывается от неё.
...
dag = account_lifetime_facts_dag()
