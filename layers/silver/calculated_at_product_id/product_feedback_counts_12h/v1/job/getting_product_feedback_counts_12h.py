...
def _validate_feedback_counts(
    ...
    invalid_rows = features.filter(
        ...
        | (F.col("feedback_gte_4") + F.col("feedback_lte_3") != F.col("feedback_count"))
        | (F.col("rating_sum") < F.lit(settings.min_rating) * F.col("feedback_count"))
        | (F.col("rating_sum") > F.lit(settings.max_rating) * F.col("feedback_count"))
    ).take(1)
    if invalid_rows:
        row = invalid_rows[0]
        raise RuntimeError(
...
def save_product_feedback_counts(
    ...
    _validate_feedback_counts(features, settings)
