"""One-off backfill: copy pairwise features from the upstream feature_store
mart into the gold legacy table for a single partition date.

This is NOT the regular gold pipeline (which is computed from silver). It exists
only to fill the gold partitions that were never produced by the Spark job, by
copying the already-validated upstream snapshot:

    iceberg.um_prod_feature_store_iceberg.query_query_skg_pairwise_features

The two sources were validated as identical on the one overlapping date
(2026-06-01): the (query, sku_group_id) key sets matched exactly (0 rows only on
either side) and every compared feature correlated at 1.0 with 100% exact
matches. Lineage for the backfilled partitions therefore differs from the
silver-computed partitions on purpose.
...
def backfill_query_skg_pairwise_features_legacy(
    spark: SparkSession,
    run_date: str,
    target_table: str,
) -> None:
...
def run(spark: SparkSession, arguments: Arguments) -> None:
    backfill_query_skg_pairwise_features_legacy(
        spark,
