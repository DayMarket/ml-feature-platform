"""Entity-local Airflow/Python runtime for the online buyout feature projection."""
...
def trino_table_name(ref: TableRef) -> str:
    ...
    if not trino_catalog:
        raise ValueError(f"No Trino catalog mapping for Iceberg catalog {ref.catalog!r}")
...
def require_non_empty(frame, partition_date: date) -> None:
    ...
    if frame.empty:
        raise RuntimeError(
            "Source query returned no rows for date="
