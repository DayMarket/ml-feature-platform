...
def _normalize_team_name(team_value: Any) -> str:
    team_name = str(team_value or "buyer")
...
def get_dag_settings() -> Dict[str, str]:
    ...
    dag_team = _normalize_team_name(
        dag_config.get("team", table_meta.get("team", "buyer"))
    )
    ...
    return {
        ...
        "group_tag": str(dag_config.get("group_tag", "buyout-features")),
        "schedule": str(dag_config.get("schedule", "0 4 * * *")),
        "start_date": str(dag_config.get("start_date", "2026-08-20T00:00:00Z")),
        "alert_severity": str(alerts_config.get("severity", "P2")),
