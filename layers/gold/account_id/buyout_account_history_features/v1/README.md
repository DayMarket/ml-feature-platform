...
<<<<<<< HEAD
`ExternalTaskSensor` на таску `dq` DAG-а silver-витрины пожизненных фактов
`feature-platform.layers.silver.account_id.account_lifetime_facts` (`external_task_id="dq"`),
=======
`ExternalTaskSensor` на таску `dq` silver-DAG'а пожизненных фактов:
`feature-platform.layers.silver.account_id.account_lifetime_facts`, `external_task_id="dq"`,
...
<<<<<<< HEAD
суток `D` и в витрину состояния на конец дня не берётся. Дельта указывает на запуск
silver-DAG-а именно партиции `D`, и сенсор ждёт его таску `dq`.
...
=======
суток `D` и в as-of витрину не берётся. Дельта попадает ровно в этот запуск:
`D 04:00 - 1 сутки 2 часа = D-1 02:00`.

Сенсор специально смотрит не на `dbt.source.trino.ml_feature_platform_silver.
feature_platform_account_lifetime_facts.dq`: у dbt-DQ-DAG'а собственное расписание `0 1 * * *`
и собственная логическая дата (`D-1 01:00`), она не совпадает с расписанием производителя ни
при какой дельте — сенсор опрашивал несуществующий ран и падал по таймауту (4 ч). Silver-DAG
считает свой DQ таской `dq` внутри себя, ждать надо её (AGENTS.md, «Downstream-DAG'и»).
>>>>>>> 738ac44 (feat: add changes)
