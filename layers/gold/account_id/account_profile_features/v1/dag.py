"""Materialize account-profile Gold features twice a day."""
...
PRODUCT_PRICES_DAG_ID = (
    "feature-platform.layers.silver.product_id.product_prices_daily"
)
DEMOGRAPHICS_DAG_ID = (
    "feature-platform.layers.silver.account_id.account_demographics"
)
ACTION_COUNTS_DAG_ID = (
    "feature-platform.layers.silver.account_id_product_id."
    "account_product_session_action_counts_12h"
)
CATEGORY_DEMOGRAPHIC_DAG_ID = (
    "feature-platform.layers.gold.category_id.category_demographic_features"
)
PRODUCT_BASE_DAG_ID = (
    "feature-platform.layers.gold.product_id.product_base_features"
)
PRODUCT_RANKING_DAG_ID = (
    "feature-platform.layers.gold.product_id.product_ranking_features"
)
...
def _daily_snapshot_logical_date(logical_date):
    ...
    raise ValueError("account-profile schedule expects a 07:00 or 19:00 UTC logical date")
...
def collect_gold_account_profile_features():
    ...
    wait_for_demographics_dq = ExternalTaskSensor(
        task_id="wait_for_silver_account_demographics_dq",
        external_dag_id=DEMOGRAPHICS_DAG_ID,
        external_task_id="dq",
        execution_date_fn=_daily_snapshot_logical_date,
        allowed_states=["success"],
        failed_states=["failed"],
    ...
    wait_for_category_demographic_dq = ExternalTaskSensor(
        task_id="wait_for_gold_category_demographic_dq",
        external_dag_id=CATEGORY_DEMOGRAPHIC_DAG_ID,
        external_task_id="dq",
        allowed_states=["success"],
        failed_states=["failed"],
        mode="reschedule",
    ...
    wait_for_product_base_dq = ExternalTaskSensor(
        task_id="wait_for_gold_product_base_dq",
        external_dag_id=PRODUCT_BASE_DAG_ID,
        external_task_id="dq",
        allowed_states=["success"],
        failed_states=["failed"],
        mode="reschedule",
    ...
    wait_for_product_ranking_dq = ExternalTaskSensor(
        task_id="wait_for_gold_product_ranking_dq",
        external_dag_id=PRODUCT_RANKING_DAG_ID,
        external_task_id="dq",
        allowed_states=["success"],
        failed_states=["failed"],
        mode="reschedule",
    ...
    materialize_task = SparkKubernetesOperator(
        ...
        task_id="getting_account_profile_features",
