"""Build the account-profile Gold snapshot and write it to Iceberg."""
...
from job.query import (
    build_account_profile_features_merge_query,
    build_account_profile_features_query,
)
...
def build_account_profile_features(
    spark: SparkSession,
    partition_end: str,
    settings: SourceSettings,
) -> DataFrame:
    calculated_at = parse_airflow_timestamp(partition_end)
    return spark.sql(build_account_profile_features_query(settings, calculated_at))


def save_account_profile_features(
    spark: SparkSession,
    partition_end: str,
    target_table: str,
) -> None:
    ...
    calculated_at = parse_airflow_timestamp(partition_end)
    features = spark.sql(build_account_profile_features_query(settings, calculated_at))
    features.createOrReplaceTempView("account_profile_features_for_calculated_at")
    spark.sql(
        build_account_profile_features_merge_query(
            target_table,
            settings,
