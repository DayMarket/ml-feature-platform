...
DAG id: `feature-platform.layers.gold.sku_group_id.sku_group_stock_features`
...
- `row_count_min: 4000000` — минимум за последние 30 партиций 5.23M строк;
- `row_count_growth: 0.2` — по 110 парам соседних партиций рост лежит в 0%..+9.55%,
  но p95 |рост| = 0.2%: 9.55% — единичный выброс, и дефолтные двусторонние 20%
  оставляют к нему запас, продолжая ловить обвал остатков.

Дополнительно `not_null` и `non_negative` по всем 8 фичевым колонкам: за 7 партиций
(около 37M строк) ни одного NULL и ни одного отрицательного значения — остаток
считается как SUM неотрицательных EOD-остатков SKU.
