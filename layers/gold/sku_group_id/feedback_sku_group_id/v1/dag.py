...
@dag(
    ...
    schedule=CronDataIntervalTimetable('10 3 * * *', 'UTC'),
    start_date=datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc),
    dag_id="feature-platform.layers.gold.sku_group_id.feedback_sku_group_id",
)
def collect_gold_sku_group_feedback_base_stats():
    collect_feedback_stats = SparkKubernetesOperator(
        execution_timeout=timedelta(hours=10),
        task_id="getting_sku_group_feedback_base_stats",
        namespace="svc-data-spark-jobs",
        application_file=get_deployment(
            ".",
            "fetch_gold_sku_group_feedback_base_stats.yaml",
        ),
    ...
    collect_feedback_stats >> [dq_task, stats_task]


dag = collect_gold_sku_group_feedback_base_stats()
