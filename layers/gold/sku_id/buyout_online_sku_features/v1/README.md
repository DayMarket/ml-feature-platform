...
Online-таблица SKU для сервиса невыкупов: одна строка на `sku_id` — выкупаемость самого
sku, его карточки, категории, магазина и бренда и сглаженные оценки.
...
и сглаживаются к родителю. В таблице все активные sku плюс все sku с доставками за 90 дней,
поэтому сервису достаточно одного поиска по `sku_id` за последнюю дату:
...
- `sku` и `product` стягиваются к сглаженной выкупаемости своей категории:
...
Сырые выкупаемости карточки, категории, магазина и бренда и число доставок
(`*_n_delivered_90d`) остаются в строке рядом со сглаженными: потребитель может
пересобрать сглаживание с другим k.

Партиция читается срезами по остатку `sku_id % shards` (`source.shards`, по умолчанию 8):
одна выгрузка на ≈9.5 млн строк не помещается в память задачи. Первый срез перезаписывает
партицию (`overwrite` с фильтром `date = D`), остальные дописываются (`append`); повтор
задачи начинается с первого среза, поэтому запись идемпотентна, а падение в середине цикла
оставляет партицию неполной до следующего успешного прогона.

## Caveats

- В таблице все активные sku (`status = 'ACTIVE'` в `silver.sku`, ≈9.5 млн) плюс все sku
  с доставками за 90 дней (≈1.5 млн, большей частью те же). Наличие на складе не проверяется:
  потребитель (`sku_buyout_features`, словарь sku сервиса) грузит партицию целиком, и у любого
  товара, который можно заказать, подстановки категории и маркетплейса должны быть посчитаны
  здесь. Без строки остаются только неактивные sku и sku, созданные после снимка D−1, —
  для них сервис берёт общую выкупаемость маркетплейса из любой строки
  (`marketplace_*_90d` одинаковы во всей партиции), число доставок 0, разрыв 0.
- `LEFT JOIN` к родителям: если у sku нет `product_id`, `shop_id` или `brand_name_id`
  в `silver.sku`, соответствующие колонки остаются `NULL`, а сглаженные выкупаемости
  считаются от `n = 0`, то есть равны выкупаемости категории.
