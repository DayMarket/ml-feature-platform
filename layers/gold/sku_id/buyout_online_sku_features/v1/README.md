...
- Популяция — только sku с историей за 90 дней (порядка 1.44 млн строк). Холодных sku в
  таблице нет: для них сервис берёт выкупаемость категории из
  [`buyout_online_category_features`](../../../category_id/buyout_online_category_features/v1/README.md)
  (см. «Подстановки» ниже) — так модель невыкупов видела такие позиции при обучении.
...
Подстановки, как при обучении:

- sku без строки (нет заказов за 90 дней): выкупаемость sku и карточки =
  `category_buyout_rate_90d` категории позиции из `buyout_online_category_features`,
  число доставок = 0, разрыв = 0;
- категории нет и там: `marketplace_buyout_rate_90d` / `marketplace_no_show_rate_90d`;
- `NULL` в родительской колонке (нет `product_id` / `shop_id` в `silver.sku`) — сглаженная
  выкупаемость уже равна выкупаемости родителя уровнем выше, дополнительно ничего
