...
@dag(
    ...
    schedule=CronDataIntervalTimetable('30 3 * * *', 'UTC'),
    start_date=datetime(2026, 6, 10, 0, 0, 0, tzinfo=timezone.utc),
    dag_id="feature-platform.layers.gold.query_sku_group_id.query_skg_pairwise_features_legacy",
)
def collect_gold_query_skg_pairwise_features_legacy():
  ...
  #  )
    ...
    collect_features = SparkKubernetesOperator(
        ...
        task_id="getting_query_skg_pairwise_features_legacy",
        namespace="svc-data-spark-jobs",
        application_file=get_deployment(
            ".",
            "fetch_gold_query_skg_pairwise_features_legacy.yaml",
        ),
