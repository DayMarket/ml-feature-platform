...
click_features AS (
    SELECT
        click.account_id,
        product.brand_id,
        CAST(SUM(CASE WHEN last_received_at >= TIMESTAMP '{calculated_at_local}' - INTERVAL 3 DAYS THEN 1 ELSE 0 END) AS INT) AS bid_n_clicks_3d,
        CAST(SUM(CASE WHEN last_received_at >= TIMESTAMP '{calculated_at_local}' - INTERVAL 7 DAYS THEN 1 ELSE 0 END) AS INT) AS bid_n_clicks_7d,
        CAST(SUM(CASE WHEN last_received_at >= TIMESTAMP '{calculated_at_local}' - INTERVAL 14 DAYS THEN 1 ELSE 0 END) AS INT) AS bid_n_clicks_14d,
        CAST(SUM(CASE WHEN last_received_at >= TIMESTAMP '{calculated_at_local}' - INTERVAL 28 DAYS THEN 1 ELSE 0 END) AS INT) AS bid_n_clicks_28d
...
brand_gmv_features AS (
    SELECT
        account_id,
        brand_id,
        CAST(SUM(CASE WHEN generated_at >= TIMESTAMP '{calculated_at_utc}' - INTERVAL 3 DAYS THEN line_gmv ELSE 0.0 END) AS DOUBLE) AS bid_gmv_3d,
        CAST(SUM(CASE WHEN generated_at >= TIMESTAMP '{calculated_at_utc}' - INTERVAL 7 DAYS THEN line_gmv ELSE 0.0 END) AS DOUBLE) AS bid_gmv_7d,
        CAST(SUM(CASE WHEN generated_at >= TIMESTAMP '{calculated_at_utc}' - INTERVAL 14 DAYS THEN line_gmv ELSE 0.0 END) AS DOUBLE) AS bid_gmv_14d,
        CAST(SUM(CASE WHEN generated_at >= TIMESTAMP '{calculated_at_utc}' - INTERVAL 28 DAYS THEN line_gmv ELSE 0.0 END) AS DOUBLE) AS bid_gmv_28d,
        CAST(SUM(CASE WHEN generated_at >= TIMESTAMP '{calculated_at_utc}' - INTERVAL 60 DAYS THEN line_gmv ELSE 0.0 END) AS DOUBLE) AS bid_gmv_60d,
        CAST(SUM(CASE WHEN generated_at >= TIMESTAMP '{calculated_at_utc}' - INTERVAL 90 DAYS THEN line_gmv ELSE 0.0 END) AS DOUBLE) AS bid_gmv_90d
...
SELECT
    ...
    entity.account_id,
    entity.brand_id,
    COALESCE(clicks.bid_n_clicks_3d, 0) AS bid_n_clicks_3d,
    COALESCE(clicks.bid_n_clicks_7d, 0) AS bid_n_clicks_7d,
    COALESCE(clicks.bid_n_clicks_14d, 0) AS bid_n_clicks_14d,
    COALESCE(clicks.bid_n_clicks_28d, 0) AS bid_n_clicks_28d,
    COALESCE(brand_gmv.bid_gmv_3d, 0.0D) AS bid_gmv_3d,
    COALESCE(brand_gmv.bid_gmv_7d, 0.0D) AS bid_gmv_7d,
    COALESCE(brand_gmv.bid_gmv_14d, 0.0D) AS bid_gmv_14d,
    COALESCE(brand_gmv.bid_gmv_28d, 0.0D) AS bid_gmv_28d,
    COALESCE(brand_gmv.bid_gmv_60d, 0.0D) AS bid_gmv_60d,
    COALESCE(brand_gmv.bid_gmv_90d, 0.0D) AS bid_gmv_90d,
    CASE WHEN account_gmv.account_gmv_3d > 0 THEN COALESCE(brand_gmv.bid_gmv_3d, 0.0D) / account_gmv.account_gmv_3d END AS bid_gmv_3d_ratio,
    CASE WHEN account_gmv.account_gmv_7d > 0 THEN COALESCE(brand_gmv.bid_gmv_7d, 0.0D) / account_gmv.account_gmv_7d END AS bid_gmv_7d_ratio,
    CASE WHEN account_gmv.account_gmv_14d > 0 THEN COALESCE(brand_gmv.bid_gmv_14d, 0.0D) / account_gmv.account_gmv_14d END AS bid_gmv_14d_ratio,
    CASE WHEN account_gmv.account_gmv_28d > 0 THEN COALESCE(brand_gmv.bid_gmv_28d, 0.0D) / account_gmv.account_gmv_28d END AS bid_gmv_28d_ratio,
    CASE WHEN account_gmv.account_gmv_60d > 0 THEN COALESCE(brand_gmv.bid_gmv_60d, 0.0D) / account_gmv.account_gmv_60d END AS bid_gmv_60d_ratio,
    CASE WHEN account_gmv.account_gmv_90d > 0 THEN COALESCE(brand_gmv.bid_gmv_90d, 0.0D) / account_gmv.account_gmv_90d END AS bid_gmv_90d_ratio
...
WHEN MATCHED THEN UPDATE SET
    target.bid_n_clicks_3d = source.bid_n_clicks_3d,
    target.bid_n_clicks_7d = source.bid_n_clicks_7d,
    target.bid_n_clicks_14d = source.bid_n_clicks_14d,
    target.bid_n_clicks_28d = source.bid_n_clicks_28d,
    target.bid_gmv_3d = source.bid_gmv_3d,
    target.bid_gmv_7d = source.bid_gmv_7d,
    target.bid_gmv_14d = source.bid_gmv_14d,
    target.bid_gmv_28d = source.bid_gmv_28d,
    target.bid_gmv_60d = source.bid_gmv_60d,
    target.bid_gmv_90d = source.bid_gmv_90d,
    target.bid_gmv_3d_ratio = source.bid_gmv_3d_ratio,
    target.bid_gmv_7d_ratio = source.bid_gmv_7d_ratio,
    target.bid_gmv_14d_ratio = source.bid_gmv_14d_ratio,
    target.bid_gmv_28d_ratio = source.bid_gmv_28d_ratio,
    target.bid_gmv_60d_ratio = source.bid_gmv_60d_ratio,
    target.bid_gmv_90d_ratio = source.bid_gmv_90d_ratio
WHEN NOT MATCHED THEN INSERT (
    ...
    source.account_id,
    source.brand_id,
    source.bid_n_clicks_3d,
    source.bid_n_clicks_7d,
    source.bid_n_clicks_14d,
    source.bid_n_clicks_28d,
    source.bid_gmv_3d,
    source.bid_gmv_7d,
    source.bid_gmv_14d,
    source.bid_gmv_28d,
    source.bid_gmv_60d,
    source.bid_gmv_90d,
    source.bid_gmv_3d_ratio,
    source.bid_gmv_7d_ratio,
    source.bid_gmv_14d_ratio,
    source.bid_gmv_28d_ratio,
    source.bid_gmv_60d_ratio,
    source.bid_gmv_90d_ratio
)
