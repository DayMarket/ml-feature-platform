"""Build one physical account-category Gold snapshot and write it to Iceberg."""
...
from job.query import (
    ...
    build_account_category_features_query,
)
...
def build_account_category_features(
    spark: SparkSession,
    partition_end: str,
    settings: SourceSettings,
) -> DataFrame:
    calculated_at = parse_airflow_timestamp(partition_end)
    return spark.sql(build_account_category_features_query(settings, calculated_at))


def save_account_category_features(
    spark: SparkSession,
    partition_end: str,
    target_table: str,
) -> None:
...
def run(spark: SparkSession, arguments: Arguments) -> None:
    save_account_category_features(
        spark,
