...
- считает сглаженные pairwise-конверсии `query_skg_smooth_conv_imp2order_{1,3,7,14,21,30,60,90}` по формуле
  `(query_skg_orders_n + 100 * skg_conv_imp2order_n) / (query_skg_impressions_n + 100)`, где
  `skg_conv_imp2order_n` считается на уровне `sku_group_id` по всем поисковым запросам за то же окно;
- считает долю ATC пары среди всех ATC `sku_group_id`:
  `query_skg_atc_frac_all_skg_atc_{1,3,7,14,21,30,60,90} = query_skg_atc_n / skg_atc_n`;
- считает долю заказов пары среди всех заказов `sku_group_id`:
  `query_skg_orders_frac_all_skg_orders_{1,3,7,14,21,30,60,90} = query_skg_orders_n / skg_orders_n`;
...
Сглаженные pairwise ATC- и order-признаки используют окна `[ds - n, ds - 1]`, то есть не включают дату расчёта. Для ATC `query_skg_atc_n` и `query_skg_impressions_n` берутся из поисковых событий пары `query, sku_group_id`, а prior `skg_conv_imp2atc_n` считается как `skg_atc_n / skg_impressions_n` на уровне `sku_group_id` из того же silver-источника поисковых событий. Для order `query_skg_orders_n` берется из `orders_generated` в `iceberg.silver.feature_platform_sku_group_query_search_orders`, а prior `skg_conv_imp2order_n` считается как `skg_orders_n / skg_impressions_n` на уровне `sku_group_id`. Коэффициент сглаживания фиксирован: `100`.

Долевые ATC-признаки `query_skg_atc_frac_all_skg_atc_*` также используют окна `[ds - n, ds - 1]`. Числитель — `sum_atc` для пары `query, sku_group_id`, знаменатель — `sum_atc` того же `sku_group_id` по всем поисковым запросам за окно. При нулевом знаменателе сохраняется Spark-семантика `NULL`.

Долевые order-признаки `query_skg_orders_frac_all_skg_orders_*` используют окна `[ds - n, ds - 1]`. Числитель — `orders_generated` для пары `query, sku_group_id`, знаменатель — `orders_generated` того же `sku_group_id` по всем поисковым запросам за окно. При нулевом знаменателе сохраняется Spark-семантика `NULL`.
...
Миграция `migrations/20260625_add_smoothed_pairwise_order.sql` идемпотентно добавляет в существующую таблицу колонки `query_skg_smooth_conv_imp2order_{1,3,7,14,21,30,60,90}`. Для новых окружений эти колонки также включены в `migrations/create_table.sql`.

## Изменение схемы от 2026-06-26

Миграция `migrations/20260626_add_atc_fraction_features.sql` идемпотентно добавляет в существующую таблицу колонки `query_skg_atc_frac_all_skg_atc_{1,3,7,14,21,30,60,90}`. Для новых окружений эти колонки также включены в `migrations/create_table.sql`.

Миграция `migrations/20260626_add_order_fraction_features.sql` идемпотентно добавляет в существующую таблицу колонки `query_skg_orders_frac_all_skg_orders_{1,3,7,14,21,30,60,90}`. Для новых окружений эти колонки также включены в `migrations/create_table.sql`.
