...
|---|---|---|
| `date` | `DATE` | дата прогона (UTC, `data_interval_start`), не дата строки витрины |
...
`D` — дата `data_interval_start` в UTC; она задаёт только партицию, в которую пишется
снимок. SQL — `render_query` в `job/getting_query_category_relevance_expanded.py`.

1. Вся витрина, без фильтра по `date`: даты источника не важны. Иначе ручной запуск,
   у которого `D` — предыдущий день, читал бы 0 строк и не писал ничего (так и было
   при первом запуске 2026-09-14).
2. К строкам добавляются копии, где `query_text` заменён на каждую формулировку того же
   `query_id` из справочника (`JOIN` только по `query_id`). Исходные строки остаются,
   в том числе те, чьего `query_id` нет в справочнике.
...
4. На пару `category_id, query_text` остаётся одна строка с максимальным `relevance`:
   `row_number() over (partition by category_id, lower(query_text) order by relevance desc nulls last) = 1`.
   Разный `relevance` у одной пары бывает, когда в категории два `query_id` отличаются
   только регистром (на 2026-09-14 — 5 пар, `faberlic` = 2 и `Faberlic` = 0), или когда
   витрина хранит пару в нескольких партициях.
