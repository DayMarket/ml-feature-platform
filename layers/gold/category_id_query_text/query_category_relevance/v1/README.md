...
Таблица была создана с `query_id` типа `BIGINT`. Iceberg меняет тип колонки только
расширяющими промоушенами, а `bigint→string` к ним не относится, поэтому
`20260914_query_id_to_string.sql` пересоздаёт колонку в три шага: уводит старую под
временное имя `query_id_bigint`, добавляет `query_id` заново строкой и удаляет
временную. Удаление здесь ничего не теряет — на момент миграции таблица пуста
(0 строк, 0 партиций), DAG-заглушка партиций не писал. После миграции схема совпадает
с `create_table.sql`, и следов промежуточной колонки не остаётся.
