...
@dag(
    ...
    max_active_runs=1,
    tags=["spark", "feature-platform", dag_settings["team_tag"], "gold", "orders"],
    is_paused_upon_creation=True,
    schedule_interval="0 3 * * *",
    start_date=datetime(2026, 6, 1, 0, 0, 0),
    dag_id="feature_platform_sku_group_search_sales_7d_gold_dag",
)
def collect_gold_sku_group_search_sales_7d():
    ...
    collect_features = SparkKubernetesOperator(
        ...
        task_id="getting_sku_group_search_sales_7d",
        namespace="svc-data-spark-jobs",
        application_file=get_deployment(
            ".",
            "fetch_gold_sku_group_search_sales_7d.yaml",
        ),
        kubernetes_conn_id="spark_k8s",
    )

    wait_for_silver_search_orders >> collect_features


dag = collect_gold_sku_group_search_sales_7d()
