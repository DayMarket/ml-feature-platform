...
Gold отдельным Trino-запросом определяет модели по raw и silver-источникам. Затем для каждого
найденного `promotion_id` берет actual dynamic_discount за последний день из
...
`created_at DESC` и считает финальные цены. Обрабатываются все модели с префиксом
`dyno_pricing_` и дефолтный `promotion_id = '0'`; список моделей в `config.yaml` не хранится.
...
Запись идет по одному `promotion_id`: для каждой найденной модели `dyno_pricing_*` и дефолтного
`0` DAG читает свой срез и перезаписывает snapshot `(calculated_at, promotion_id)` через PyIceberg
`overwrite`. Каждый Trino-срез dynamic_discount явно ограничен через
`starts_with(promotion_id, 'dyno_pricing_')`.
