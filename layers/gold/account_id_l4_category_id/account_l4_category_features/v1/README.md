# Account-L4-category features

DAG id: feature-platform.layers.gold.account_id_l4_category_id.account_l4_category_features.
...
Целевая таблица: iceberg.gold.feature_platform_account_l4_category_features.

## Контракт

Путь сущности: layers/gold/account_id_l4_category_id/account_l4_category_features/v1.

Grain и primary key: calculated_at,account_id,l4_category_id.

Идентификаторы и счётчики в физическом контракте имеют тип INT.

Namespace контракта — `ACCOUNT_L4`. Все физические feature-колонки уже содержат
его, например `ACCOUNT_L4__n_clicks_7d`; устаревший префикс `l4_` не
используется. Ключи `calculated_at`, `account_id` и `l4_category_id` остаются
без namespace.
...
l4_category_id в G2 не попадают.

## Actions

Источник: iceberg.silver.feature_platform_account_product_session_action_counts_12h.

Перед агрегацией строки S2c повторно дедуплицируются на полном 28-дневном
окне по account_id,session_id,product_id,event_type; n_events не суммируется.
...
До записи DAG ждёт dq владельцев S1 и S2c. Для
...
а также ratios в диапазоне [0,1]. Пороги freshness и объёма на первичной
раскатке имеют severity warn. Alert callbacks остаются отключёнными до окончания
отладки.
